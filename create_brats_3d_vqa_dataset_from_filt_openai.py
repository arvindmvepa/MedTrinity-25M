...
def validate_question_answer_combo(question, answer_template, combo):
    flag = True
    # removing this so RNG is not messed up by the filtering
    # flag = flag and "{label}" in question and "{label}" in answer_template
...
question_types=("area", "region", "shape", "satellite", "partially_unknown", "unknown")):
    ...
    for seg_id, labels_question_types_vqa_datum in tqdm(vqa_data_dict.items()):
        ...
        for label, question_types_vqa_datum in labels_question_types_vqa_datum.items():
            ...
            # -------------------------------------------
            qas = pick_num_question_types_combos_and_rows(df=openai_df, filt_df=openai_filt_df, rng=rng)
...
if __name__ == "__main__":
    ...
    openai_partially_unknown_filt_df_file = "mri_dataset_partially_unknown_combined1_clean_validity7.csv"
    ...
    openai_unknown_filt_df = map_df_cols_to_unknown(openai_unknown_filt_df)
    openai_unknown_filt_df = openai_unknown_filt_df.sample(frac=1, random_state=new_dataset_seed)
