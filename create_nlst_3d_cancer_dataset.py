...
def summarize_vqa(final_vqa, na_string="NA", nan_string="nan", sep_string="|", print_inst_results=False):
    ...
    n_year0 = (df["study_yr"] == 0).sum()
    n_year1 = (df["study_yr"] == 1).sum()
    n_year2 = (df["study_yr"] == 2).sum()


    # statistics on different question types
    cancer_counts = df.loc[df['content_type'] == 'cancer']['answer'].value_counts()

    n_pids = df["pid"].nunique()

    print("=== Overall Statistics ===")
    print(f"Total number of questions: {n_questions}")
    print(f"Number of questions with year 0: {n_year0}")
    print(f"Number of questions with year 1: {n_year1}")
    print(f"Number of questions with year 2: {n_year2}")


    print(f"Value Counts for cancer_counts: {cancer_counts}")
...
def build_question(question, answer, pid, study_yr, inst, embedding_path, question_index, content_type):
    """
...
def get_questions(cur_rows, next_rows, pid, study_yr, inst, question_index, embedding_path):
    ...
    cancer_qa = build_question(
        ...
        study_yr=study_yr,
        inst=inst,
        question=f"Will this patient develop cancer?",
        answer="yes" if has_cancer else "no",
        embedding_path=embedding_path,
        question_index=question_index,
        content_type="cancer"
    )
    q_list.append(cancer_qa)
    question_index += 1
...
def generate_vqa_from_df(ann_df, add_time_delta2=False, save_dir="/hsuraid/avepa/nlst_sybil_embeddings"):
    ...
    for pid, pid_ann_df in tqdm(ann_df.groupby('pid')):
        ...
        if os.path.exists(embedding_path):
            ...
            all_vqas.extend(qas)
        # create t2 question
        embedding_path = os.path.join(save_dir, f"pid{pid}_ts2.st")
        if os.path.exists(embedding_path):
